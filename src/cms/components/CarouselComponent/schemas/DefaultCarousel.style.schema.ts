import { StyleSchema } from '../../../../lib/schema-registry.js';
import { CarouselComponentDefinition } from './Carousel.type.schema.js';

// Auto-registering Carousel style schema
export const DefaultCarouselStyleDefinition = StyleSchema({
  key: 'DefaultCarousel',
  displayName: 'Carousel',
  contentType: CarouselComponentDefinition.key,
  isDefault: true,
  settings: {
      carouselMode: {
        displayName: 'Carousel Mode',
        editor: '',
        sortOrder: 10,
        choices: {
          standard: {
            displayName: 'Standard (Full Width)',
            sortOrder: 10,
          },
          content: {
            displayName: 'Content (Multiple Items)',
            sortOrder: 20,
          },
        },
      },
      itemsPerViewDesktop: {
        displayName: 'Items Per View (Desktop)',
        editor: '',
        sortOrder: 20,
        choices: {
          items2: {
            displayName: '2 items',
            sortOrder: 10,
          },
          items3: {
            displayName: '3 items',
            sortOrder: 20,
          },
          items4: {
            displayName: '4 items',
            sortOrder: 30,
          },
          items5: {
            displayName: '5 items',
            sortOrder: 40,
          },
        },
      },
      itemsPerViewTablet: {
        displayName: 'Items Per View (Tablet)',
        editor: '',
        sortOrder: 30,
        choices: {
          items1: {
            displayName: '1 item',
            sortOrder: 10,
          },
          items2: {
            displayName: '2 items',
            sortOrder: 20,
          },
          items3: {
            displayName: '3 items',
            sortOrder: 30,
          },
          items4: {
            displayName: '4 items',
            sortOrder: 40,
          },
        },
      },
      itemsPerViewMobile: {
        displayName: 'Items Per View (Mobile)',
        editor: '',
        sortOrder: 40,
        choices: {
          items1: {
            displayName: '1 item',
            sortOrder: 10,
          },
          items2: {
            displayName: '2 items',
            sortOrder: 20,
          },
          items3: {
            displayName: '3 items',
            sortOrder: 30,
          },
        },
      },
      spaceBetween: {
        displayName: 'Space Between Items',
        editor: '',
        sortOrder: 50,
        choices: {
          space16: {
            displayName: 'Small (16px)',
            sortOrder: 10,
          },
          space24: {
            displayName: 'Medium (24px)',
            sortOrder: 20,
          },
          space32: {
            displayName: 'Large (32px)',
            sortOrder: 30,
          },
        },
      },
      showNavigation: {
        displayName: 'Show Navigation Arrows',
        editor: '',
        sortOrder: 60,
        choices: {
          true: {
            displayName: 'Yes',
            sortOrder: 10,
          },
          false: {
            displayName: 'No',
            sortOrder: 20,
          },
        },
      },
      showPagination: {
        displayName: 'Show Pagination Dots',
        editor: '',
        sortOrder: 70,
        choices: {
          true: {
            displayName: 'Yes',
            sortOrder: 10,
          },
          false: {
            displayName: 'No',
            sortOrder: 20,
          },
        },
      },
      autoplay: {
        displayName: 'Enable Autoplay',
        editor: '',
        sortOrder: 80,
        choices: {
          true: {
            displayName: 'Yes',
            sortOrder: 10,
          },
          false: {
            displayName: 'No',
            sortOrder: 20,
          },
        },
      },
      autoplayDelay: {
        displayName: 'Autoplay Delay (seconds)',
        editor: '',
        sortOrder: 90,
        choices: {
          delay3s: {
            displayName: '3 seconds',
            sortOrder: 10,
          },
          delay5s: {
            displayName: '5 seconds',
            sortOrder: 20,
          },
          delay7s: {
            displayName: '7 seconds',
            sortOrder: 30,
          },
          delay10s: {
            displayName: '10 seconds',
            sortOrder: 40,
          },
        },
      },
      loop: {
        displayName: 'Enable Loop',
        editor: '',
        sortOrder: 100,
        choices: {
          true: {
            displayName: 'Yes',
            sortOrder: 10,
          },
          false: {
            displayName: 'No',
            sortOrder: 20,
          },
        },
      },
      carousel_height: {
        displayName: 'Carousel Height',
        editor: '',
        sortOrder: 110,
        choices: {
          h_auto: {
            displayName: 'Auto (Content Height)',
            sortOrder: 10,
          },
          h_64: {
            displayName: 'Small (16rem)',
            sortOrder: 20,
          },
          h_80: {
            displayName: 'Medium (20rem)',
            sortOrder: 30,
          },
          h_96: {
            displayName: 'Large (24rem)',
            sortOrder: 40,
          },
          h_112: {
            displayName: 'X-Large (28rem)',
            sortOrder: 50,
          },
          h_128: {
            displayName: 'XX-Large (32rem)',
            sortOrder: 60,
          },
          h_144: {
            displayName: 'XXX-Large (36rem)',
            sortOrder: 70,
          },
          h_screen: {
            displayName: 'Full Screen',
            sortOrder: 80,
          },
        },
      },
      image_fit: {
        displayName: 'Image Fit',
        editor: '',
        sortOrder: 120,
        choices: {
          object_cover: {
            displayName: 'Cover (Fill Container, May Crop)',
            sortOrder: 10,
          },
          object_contain: {
            displayName: 'Contain (Show Full Image, May Have Gaps)',
            sortOrder: 20,
          },
          object_fill: {
            displayName: 'Fill (Stretch to Fit)',
            sortOrder: 30,
          },
        },
      },
      carousel_alignment: {
        displayName: 'Carousel Alignment',
        editor: '',
        sortOrder: 130,
        choices: {
          full_width: {
            displayName: 'Full Width',
            sortOrder: 10,
          },
          centered_large: {
            displayName: 'Centered (Large)',
            sortOrder: 20,
          },
          centered_medium: {
            displayName: 'Centered (Medium)',
            sortOrder: 30,
          },
          centered_small: {
            displayName: 'Centered (Small)',
            sortOrder: 40,
          },
        },
      },
  },
});

// Function to serialize to JSON (returns a copy that can be safely modified)
export function serializeDefaultCarouselStyleToJSON() {
  return JSON.parse(JSON.stringify(DefaultCarouselStyleDefinition));
}
