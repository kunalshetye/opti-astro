// /src/pages/opti-astro-dach1/src/pages/opti-admin/api/reporting/content-lifecycle.json.ts
...
export const GET: APIRoute = async ({ url }) => {
  try {
    const inactivityPeriod = parseInt(url.searchParams.get('inactivityPeriod') || '90');
    const inactivityUnit = url.searchParams.get('inactivityUnit') || 'days';
    ...
    // GraphQL query to fetch stale content with variations
    const pageQuery = `
    ...
    // GraphQL query to fetch stale experiences with variations
    const experienceQuery = `
      query GetStaleContentExperiences {
        _Experience(
          limit: 100
          variation: { include: ALL }
          orderBy: {
